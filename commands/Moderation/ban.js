...
exports.run = async (bot, message, args, functions) => {
    ...
    if (member.hasPermission("ADMINISTRATOR") || member.hasPermission("BAN_MEMBERS")) {
        ...
        if (!target) return functions.errorEmbed(message, message.channel, "Please mention someone to ban.")
        const targetMember = message.guild.members.cache.get(target.id)

        const successEmbed = new Discord.MessageEmbed()
            .setColor("#64b386")
            .setDescription(`**<:CheckMark:870465575552434217> ${target.user.tag} was banned** | ${reason}`)
            .setTimestamp()
...
exports.help = {
    name: "ban",
    aliases: [],
    description: 'Bans someone in the server.'
}